...
const populateTrackData =
  require("../constants/trackInformation").populateTrackData;
...
async function createSkills() {
  ...
  await Promise.all([
    skillCreate(0, "Velocity", "#ffff4d"),
    skillCreate(1, "Swim", "#1a53ff"),
    skillCreate(2, "Jump", "#ff9900"),
    skillCreate(3, "Balance", "#009933"),
    skillCreate(4, "Might", "#cc0000"),
    skillCreate(5, "Instinct", "#6600cc"),
    skillCreate(6, "None", "#47476b"),
  ]);
}

async function skillCreate(index, name, colour) {
...
async function createTracks() {
  ...
  /**
   * Track templates are created using the skills aray generated
   */

  const trackData = populateTrackData(skills);

  const promises = [];
  trackData.forEach((track, index) => {
    promises.push(trackCreate(index, track));
  });
  await Promise.all(promises);
}

async function trackCreate(index, trackData) {
  const track = new Track({
    index: index,
    display_order: trackData.display_order,
    name: trackData.name,
    trial_track: trackData.trial_track,
    draw_array: trackData.draw_array,
    obstacle_sequence: trackData.obstacle_sequence,
  });
  await track.save();
  tracks[index] = track;
}
